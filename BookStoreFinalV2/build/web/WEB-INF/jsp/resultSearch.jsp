    ...
    <body>
        ...
        <div class="col-md-10  panel panel-default" style="margin-top: 30px">
            ...
            <table class="table">
                ...
                <tbody>
                    <c:forEach items="${listResult}" var="rb" >
                        <tr>
                            <td>${rb.isbn}</td>
                            <td>${rb.bookName}</td>
                            <td><fmt:formatNumber value="${rb.price}" /></td>
                            <td><fmt:formatNumber value="${rb.discount}" /></td>
                            <td>${rb.bookCategoryId.name}</td>
                            <td>${rb.authorId.name}</td>
                            <c:set var = "status" scope = "session" value = "${rb.status}"/>
                            <c:if test = "${status == 0}">
                            ...
                            <td>${rb.publisherId.name}</td>
                            <td>${rb.publishYear}</td>
                            <td><a href="detailB.htm?isbn=${rb.isbn}" class="btn btn-info btn-sm">View Info</a></td>
                            <td><a href="deleteB.htm?isbn=${rb.isbn}" class="btn btn-danger btn-sm" onclick="return alert('Are You Sure?')">Delete</a></td>
                        </tr>
                    </c:forEach>
                        <tr></tr>
                </tbody>
            ...
            <div>
                <mytag:PaginationTaglib max="10" steps="10" offset="${offset}" count="${count}" 
                                        uri="resultSearch.htm" next="&raquo;" previous="&laquo;"/>
            </div>
